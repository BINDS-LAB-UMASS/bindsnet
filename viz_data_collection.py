...
if not os.path.exists("./viz/states/"):
    os.makedirs("./viz/states/")

experiment_dir = os.path.abspath("./viz/{}".format(environment.env.spec.id))
...
for i_episode in range(num_episodes):
    ...
    for t in itertools.count():
        ...
        ann_vals = dqn_network(torch.sum(state, dim=2).view([1, -1]).cuda())[0].cpu().detach().numpy()
        if ann_values is not None:
            ann_values = np.concatenate((ann_values, np.expand_dims(ann_vals, axis=0)))
        else:
            ann_values = np.expand_dims(ann_vals, axis=0)

        # save the data here
        environment.env.env.env.ale.saveScreenPNG("viz/frames/" + str(t) + ".png")
        np.savetxt('viz/states/' + str(t) + '.txt', torch.sum(state, dim=2).numpy())
        if spikes_data is not None:
            spikes_data = np.concatenate((spikes_data, np.expand_dims(readout_spikes.cpu().numpy(), axis=0)))
        else:
            spikes_data = np.expand_dims(readout_spikes.cpu().numpy(), axis=0)
...
pickle.dump(spikes_data, open("viz/spikes_data.txt", "wb"))
pickle.dump(ann_values, open("viz/ann_values.txt", "wb"))
np.savetxt("viz/selected_actions.txt", np.array(selected_actions))
