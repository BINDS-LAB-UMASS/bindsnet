...
for i_episode in range(num_episodes):
    ...
    for t in itertools.count():
        ...
        if isConvNet:
            states_batch = [torch.sum(torch.tensor([0.25, 0.5, 0.75, 1]) * state.cuda(), dim=2).unsqueeze(0).unsqueeze(0) for state in states_batch]
            next_states_batch = [torch.sum(torch.tensor([0.25, 0.5, 0.75, 1]) * state.cuda(), dim=2).unsqueeze(0).unsqueeze(0) for state in next_states_batch]
        else:
