...
exc = LIFNodes(n=hidden_neurons, refrac=0, traces=True, thresh=-52, rest=-65.0, decay=1e-2,
               probabilistic=probabilistic, beta=beta)  # Excitatory layer
...
readout = LIFNodes(n=4, refrac=0, traces=True, thresh=-52.0, rest=-65.0, decay=1e-2,
                   probabilistic=probabilistic, beta=beta)  # Readout layer
...
experiment_dir = os.path.abspath("./snn/probparamsearch/"+str(layer1scale)+"x"+str(layer2scale)+"x_beta"+str(beta)+"/{}".format(environment.env.spec.id))
...
np.savetxt('analysis/snn_tgd_param_prob_'+str(layer1scale)+'x'+str(layer2scale)+'x_beta'+str(beta)+'.txt', episode_rewards)
